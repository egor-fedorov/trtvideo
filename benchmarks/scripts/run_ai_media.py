#!/usr/bin/env python3
"""Run the canonical ai-media-enhancer benchmark workload."""
...
def build_command(args: argparse.Namespace, manifest: dict[str, Any]) -> list[str]:
    ...
    command = [
        ...
        "--warmup-frames",
        str(args.warmup_frames or benchmark["warmup_frames"]),
        "--frames",
        str(args.frames or benchmark["measured_frames"]),
        "--runs",
        str(args.runs or benchmark["initial_runs"]),
        "--extra-runs",
        str(
            args.extra_runs
