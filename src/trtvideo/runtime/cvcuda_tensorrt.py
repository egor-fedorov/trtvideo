...
class CvcudaTensorRTRuntime:
    ...
    def __init__(
        ...
        if self.engine is None:
            print(f"ERROR: Failed to load engine: {engine_path}", file=self.log_stream)
            print(
                "  Engine may be built with a different TensorRT version "
                f"(current: {trt.__version__}).",
                file=self.log_stream,
            )
            print("  Rebuild: build-engine <model>.onnx", file=self.log_stream)
            sys.exit(1)
        ...
        try:
            ...
            print(f"ERROR: Unsupported TensorRT engine contract: {exc}", file=self.log_stream)
            print(
                "  Current video runtime supports fp32/fp16 RGB tensor bindings only.",
                file=self.log_stream,
            )
            sys.exit(1)
        ...
        try:
            ...
            print(f"ERROR: Unsupported TensorRT engine contract: {exc}", file=self.log_stream)
            print(
                "  Current video runtime supports static single-frame RGB upscale engines only.",
                file=self.log_stream,
            )
            print(
                "  For video inference, build a static engine from a static ONNX variant.",
                file=self.log_stream,
            )
            sys.exit(1)
        ...
        if not quiet:
            print(f"  Engine: {engine_path}", file=self.log_stream)
            print(f"  GPU:    {self.gpu_name} (cuda:{gpu_id})", file=self.log_stream)
            print(
                f"  Input:  {self.input_w}x{self.input_h} {self.model_spec.inputs[0].dtype}",
                file=self.log_stream,
            )
            print(
                f"  Output: {self.output_w}x{self.output_h} {self.model_spec.outputs[0].dtype}",
                file=self.log_stream,
            )
            print(f"  Scale:  {self.model_spec.scale}x", file=self.log_stream)
            print("  Tensor buffers: CV-CUDA (PyTorch-free)", file=self.log_stream)
