...
def _advcvtcolor_into(dst, src, code, color_spec: str, stream=None) -> None:
    spec = _resolve_color_spec(color_spec)
    if spec is None or not hasattr(cvcuda, "advcvtcolor_into"):
        cvcuda.cvtcolor_into(dst, src, code, stream=stream)
        return
    cvcuda.advcvtcolor_into(dst, src, code, spec, stream=stream)


def nv12_to_rgb_into(
    nv12,
    height,
    width,
    rgb_buf,
    nv12_buf=None,
    color_spec: str = "bt709",
    stream=None,
):
...
def nv12_to_rgb(nv12, height, width, color_spec: str = "bt709", stream=None):
...
def rgb_to_nv12_into(rgb, nv12_buf, color_spec: str = "bt709", stream=None):
...
def rgb_to_nv12(rgb, color_spec: str = "bt709", stream=None):
    ...
    return rgb_to_nv12_into(rgb, nv12_buf, color_spec=color_spec, stream=stream)
